...
@router.get('/tables')
async def list_tables(request: Request, current_user_id: str = Depends(get_current_user_id_from_session)):
    return admin_db.list_tables(current_user_id)


@router.get('/tables/{table_name}/columns')
async def table_columns(request: Request, table_name: str, current_user_id: str = Depends(get_current_user_id_from_session)):
...
async def migration_draft(request: Request, payload: dict, current_user_id: str = Depends(get_current_user_id_from_session)):
